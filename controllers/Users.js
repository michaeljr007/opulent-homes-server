...
const signup = [
  ...
  check("code").notEmpty().withMessage("Please provide signup code"),
  check("role").notEmpty().withMessage("Please provide a role"),
  check("email")
    .isEmail()
  ...
  async (req, res) => {
    ...
    const { firstname, lastname, phone, email, password, gender, code, role } =
      req.body;
    ...
    if (code) {
      ...
      if (codeEntry) {
        ...
        return res
          .status(400)
          .json({ msg: "Invalid or already used secret code" });
      }
    }
